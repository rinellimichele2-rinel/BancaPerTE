...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/transactions/:userId/expense-templates", async (req, res) => {
    ...
    const manualExpenses = transactions.filter(
      (t: { type: string; isSimulated: boolean }) =>
        t.type === "expense" && t.isSimulated === false,
    );

    // Extract unique descriptions with their categories
    const templates = new Map<
      string,
      { description: string; category: string; avgAmount: number }
    >();
  ...
  app.get("/api/users/search/:username", async (req, res) => {
    ...
    if (user.isBlocked) {
      return res
        .status(403)
        .json({ error: "Questo utente non può ricevere trasferimenti" });
    }
  ...
  app.get("/api/admin/users", async (req, res) => {
    ...
    const sortBy =
      (req.query.sortBy as string) === "balance" ? "balance" : "newest";
  ...
  app.post("/api/bonifico", async (req, res) => {
    ...
    const trn = `0${Math.floor(Math.random() * 1000000000000000000)
      .toString()
      .substring(0, 18)}`;
